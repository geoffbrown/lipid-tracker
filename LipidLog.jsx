...
/* ════════════════════════════════════════════════════════════════════════════
   DASHBOARD
   ════════════════════════════════════════════════════════════════════════════ */
function DashboardView({ enriched, settings, onSelect, onAdd, onViewAll }) {
  const t = useT();
  ...
  const latestL = latest ? getDispLDL(latest,latest.d,settings.ldlMethod) : null;
  ...
  return (
    <div style={{paddingBottom:16}}>
      {latest && (()=>{
        const cards = [
          latestL && {label:"LDL", value:latestL.value, provenance:latestL.label,
            color:"#DF3F2E", trend:trendFor("ldl"), lowerBetter:true},
          latest.hdl!=null && {label:"HDL", value:latest.hdl, provenance:"Measured",
            color:"#1B9E5A", trend:trendFor("hdl"), lowerBetter:false},
          latest.d.apob!=null && {label:"ApoB", value:latest.d.apob, provenance:latest.d.apobLabel,
            color:"#7B4FD4", trend:trendFor("apob"), lowerBetter:true},
        ].filter(Boolean);
        if (!cards.length) return null;
        return (
          <div style={{marginTop:16,display:"grid",
            gridTemplateColumns:`repeat(${cards.length},1fr)`,gap:8}}>
