  ...
  return (
    <div onClick={e=>e.target===e.currentTarget&&onCancel()}
      ...
      <div style={{background:t.card,borderRadius:16,padding:"22px 22px 18px",maxWidth:340,
        ...
        <div style={{fontSize:17,color:t.sec,lineHeight:1.5,marginBottom:20}}>{message}</div>
        <div style={{display:"flex",gap:10}}>
          <button onClick={onCancel} style={{flex:1,padding:"11px",borderRadius:10,
            ...
            fontWeight:600,fontSize:17,cursor:"pointer",fontFamily:FONT}}>Cancel</button>
          <button onClick={onConfirm} style={{flex:1,padding:"11px",borderRadius:10,border:"none",
            ...
            fontWeight:700,fontSize:17,cursor:"pointer",fontFamily:FONT}}>{confirmLabel}</button>
        </div>
      </div>
    </div>
  );
    ...
    <div onClick={e=>e.target===e.currentTarget&&onCancel()}
      ...
      <div style={{background:t.card,borderRadius:16,padding:"22px",maxWidth:340,width:"100%",
        ...
        <input autoFocus value={val} placeholder={placeholder}
          ...
          style={{width:"100%",padding:"11px 13px",borderRadius:9,fontSize:17.5,boxSizing:"border-box",
            border:`1px solid ${err?t.danger:t.border}`,background:t.bg,color:t.text,
            outline:"none",fontFamily:FONT,marginBottom:err?6:16}} />
        {err && <div style={{fontSize:16,color:t.danger,marginBottom:14}}>{err}</div>}
        <div style={{display:"flex",gap:10}}>
          <button onClick={onCancel} style={{flex:1,padding:"11px",borderRadius:10,
            border:`1px solid ${t.border}`,background:"transparent",color:t.text,
            fontWeight:600,fontSize:17,cursor:"pointer",fontFamily:FONT}}>Cancel</button>
          <button onClick={submit} style={{flex:1,padding:"11px",borderRadius:10,border:"none",
            background:t.accent,color:t.accentText,fontWeight:700,fontSize:17,
...
function Toast({ message }) {
  ...
  return (
    <div style={{position:"fixed",bottom:96,left:"50%",transform:"translateX(-50%)",zIndex:400,
      ...
      padding:"10px 18px",borderRadius:10,fontSize:17,fontWeight:600,fontFamily:FONT,
...
function SourceBadge({ source }) {
  ...
  return (
    <span style={{background:`${m.color}1F`,color:m.ink[t.isDark?"dark":"light"],
      ...
      fontSize:15,fontWeight:700,letterSpacing:"0.3px"}}>{m.label}</span>
  );
}
...
function Stepper({ value, onUp, onDown }) {
  ...
  return (
    <div style={{display:"flex",alignItems:"center"}}>
      ...
      <div style={{fontSize:17.5,fontWeight:600,fontFamily:MONO,color:t.text,minWidth:36,height:30,
...
function DateTimePicker({ value, onChange }) {
  ...
  const field = { width:"100%",padding:"10px 12px",borderRadius:8,boxSizing:"border-box",
    border:`1px solid ${open?t.accent:t.border}`,background:t.bg,fontSize:17.5,
        ...
        <div style={{overflow:"hidden",minHeight:0}}>
          <div style={{marginTop:8,border:`1px solid ${t.border}`,borderRadius:13,
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
              ...
              <span style={{fontSize:16.5,fontWeight:700,color:t.text}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)"}}>
              {DOW.map((d,i)=>(
                <div key={i} style={{textAlign:"center",fontSize:15,fontWeight:600,color:t.muted,
                ...
                <div key={i} style={{height:28}}>
                  {d && (
                    <button onClick={()=>pickDay(d)} style={{width:"100%",height:"100%",border:"none",
                      cursor:"pointer",fontFamily:MONO,fontSize:16,borderRadius:6,
            ...
            <div style={{display:"flex",alignItems:"center",gap:10,marginTop:10,paddingTop:10,
              ...
              <span style={{fontSize:17.5,fontWeight:700,color:t.muted}}>:</span>
            ...
            <div style={{display:"flex",gap:8,marginTop:10}}>
              <button onClick={()=>{ apply(new Date()); setView({y:new Date().getFullYear(),m:new Date().getMonth()}); }}
                style={{flex:1,padding:"7px",borderRadius:8,border:`1px solid ${t.border}`,
                  background:"transparent",color:t.text,fontWeight:600,fontSize:16.5,
                  cursor:"pointer",fontFamily:FONT}}>Now</button>
              <button onClick={()=>setOpen(false)}
                style={{flex:1,padding:"7px",borderRadius:8,border:"none",background:t.accent,
                  color:t.accentText,fontWeight:700,fontSize:16.5,cursor:"pointer",fontFamily:FONT}}>Done</button>
            </div>
          </div>
        </div>
      </div>
...
function ChartTooltip({ active, payload, label, bmA, bmB }) {
  ...
  return (
    <div style={{background:t.card,border:`1px solid ${t.borderHi}`,borderRadius:9,
      ...
      <div style={{fontSize:15,color:t.sec,textTransform:"uppercase",letterSpacing:".4px",
        marginBottom:6}}>
        {fmtDate(label)}
      ...
      {av?.value!=null && (
        <div style={{display:"flex",alignItems:"baseline",gap:4,marginBottom:bv?4:0}}>
          ...
          <span style={{fontSize:15,color:t.sec}}>{a?.label}</span>
        </div>
      )}
      {bv?.value!=null && (
        <div style={{display:"flex",alignItems:"baseline",gap:4}}>
          <span style={{fontSize:18,fontWeight:600,color:b?.color,fontFamily:MONO}}>{bv.value}</span>
          <span style={{fontSize:15,color:t.sec}}>{b?.label}</span>
        </div>
      )}
    </div>
  );
    ...
    <div onClick={() => onSelect(reading)}
      ...
      <div style={{flex:1,minWidth:0}}>
        <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:7,minWidth:0}}>
          ...
          {reading.sourceName && (
            <span style={{fontSize:15,color:t.sec,overflow:"hidden",whiteSpace:"nowrap",
        ...
        <div style={{display:"flex",gap:18,alignItems:"baseline",overflow:"hidden"}}>
          {shown.map(m=>(
            <span key={m.key} style={{display:"inline-flex",alignItems:"baseline",gap:5,
              whiteSpace:"nowrap"}}>
              <span style={{fontSize:15,color:t.sec}}>{m.label}</span>
              <span style={{fontSize:17.5,fontWeight:600,color:t.text,
    ...
    <div style={{padding:"4px 20px 28px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",padding:"8px 0 16px"}}>
        <div>
          ...
          <div style={{fontSize:16.5,color:t.sec,marginTop:4,display:"flex",alignItems:"center",gap:7}}>
          ...
          <div key={i} style={{padding:m.sm?"8px 14px":"11px 14px",
            ...
            <div style={{textAlign:"right",flexShrink:0}}>
              ...
              {!m.sm && m.key && REF_RANGES[m.key] && (
                <div style={{fontSize:15,color:t.muted,marginTop:2,fontFamily:FONT}}>{REF_RANGES[m.key]}</div>
              )}
            </div>
          </div>
        ))}
      ...
      {ldl?.blocked && (
        <div style={{marginTop:10,padding:"10px 13px",background:t.bg,borderRadius:11,
          border:`1px dashed ${t.border}`,fontSize:15.5,color:t.sec,lineHeight:1.5}}>
          {ldl.blockedReason}
        </div>
      )}
      <div style={{fontSize:15,color:t.muted,margin:"7px 4px 0"}}>
      ...
      {settings.lpa && (
        <div style={{marginTop:12,padding:"11px 14px",background:t.bg,borderRadius:13,
          ...
          <div>
            <div style={{fontSize:16.5,color:t.text}}>Lp(a)</div>
            <div style={{fontSize:15,color:t.muted,marginTop:1}}>Genetic baseline · persistent</div>
          </div>
          <span style={{fontSize:17.5,fontWeight:600,color:t.text,fontFamily:MONO}}>
      ...
      {reading.notes && (
        <div style={{marginTop:12,padding:"11px 14px",background:t.bg,borderRadius:13,border:`1px solid ${t.border}`}}>
          ...
          <div style={{fontSize:17,color:t.text,lineHeight:1.55}}>{reading.notes}</div>
        </div>
      )}
      <div style={{display:"flex",gap:10,marginTop:16}}>
        <button onClick={onEdit} style={{flex:1,padding:"12px",borderRadius:10,border:"none",
          cursor:"pointer",background:t.accent,color:t.accentText,fontWeight:700,fontSize:17.5,fontFamily:FONT}}>
          Edit</button>
        <button onClick={()=>setConfirm(true)} style={{flex:1,padding:"12px",borderRadius:10,fontFamily:FONT,
          ...
          fontWeight:600,fontSize:17.5,display:"flex",alignItems:"center",justifyContent:"center",gap:5}}>
      ...
      <div style={{borderRadius:13,overflow:"hidden",border:`1px solid ${t.border}`}}>
        {options.map((o,i)=>(
          <div key={o.value} onClick={()=>onSelect(o.value)}
            ...
            <div>
              <div style={{fontSize:17,color:t.text,fontWeight:o.value===current?700:500}}>{o.label}</div>
              {o.sub && <div style={{fontSize:15.5,color:t.sec,marginTop:2}}>{o.sub}</div>}
            </div>
      ...
      <div style={{width:"100%",maxWidth:600,background:t.card,borderRadius:"22px 22px 0 0",
        ...
        <div style={{position:"relative",marginBottom:14}}>
          <button onClick={close} style={{position:"absolute",top:0,right:0,padding:"6px 10px",
            ...
            fontSize:16.5,cursor:"pointer",fontFamily:FONT}}>Done</button>
          <div style={{textAlign:"center",paddingTop:4}}>
            <div style={{fontSize:15.5,color:t.sec,fontWeight:600,letterSpacing:".3px"}}>{field.label}</div>
            <div style={{fontSize:44,fontWeight:700,fontFamily:MONO,color:t.text,lineHeight:1.1,
              marginTop:3}}>
              {val===""?<span style={{color:t.muted}}>0</span>:val}
              <span style={{fontSize:17,color:t.muted,marginLeft:7,fontFamily:FONT,fontWeight:500}}>mg/dL</span>
            </div>
          </div>
        </div>
        ...
        <div style={{display:"flex",gap:8}}>
          <button onClick={goPrev} disabled={idx<=0}
            style={{flex:1,padding:"11px",borderRadius:10,border:`1px solid ${t.border}`,
              background:"transparent",color:t.text,fontWeight:600,fontSize:17,fontFamily:FONT,
          ...
          <button onClick={goNext}
            style={{flex:1,padding:"11px",borderRadius:10,border:"none",background:t.accent,
              color:t.accentText,fontWeight:700,fontSize:17,fontFamily:FONT,cursor:"pointer"}}>
...
function AddEditModal({ reading, settings, onSave, onClose }) {
  ...
  const iS = { width:"100%",padding:"11px 13px",borderRadius:9,border:`1px solid ${t.border}`,
    background:t.bg,fontSize:17.5,outline:"none",boxSizing:"border-box",fontFamily:FONT,color:t.text };
  const lS = { fontSize:16.5,fontWeight:500,color:t.sec,display:"block",marginBottom:6 };
      ...
      <div style={{padding:"4px 22px 0"}}>
        ...
        {errors.length>0 && (
          <div style={{background:t.dangerBg,border:`1px solid ${t.danger}44`,borderRadius:9,
            ...
            {errors.map((e,i)=><div key={i} style={{fontSize:16.5,color:t.danger}}>{e}</div>)}
          </div>
        )}
        {warnings.length>0 && (
          <div style={{background:t.warnBg,border:`1px solid ${t.warn}44`,borderRadius:9,
            padding:"10px 14px",marginBottom:14}}>
            <div style={{fontSize:15.5,fontWeight:600,color:t.warn,marginBottom:3}}>Heads up: you can still save</div>
            {warnings.map((w,i)=><div key={i} style={{fontSize:16.5,color:t.warn}}>{w}</div>)}
          </div>
        )}
        <div style={{display:"flex",flexDirection:"column",gap:15}}>
            ...
            <div style={{display:"flex",gap:8}}>
              {[["POC","Home Test"],["Lab","Lab Test"]].map(([v,lbl])=>(
                <button key={v} onClick={()=>setSource(v)} style={{flex:1,padding:"11px",borderRadius:9,
                  fontFamily:FONT,fontWeight:600,fontSize:17,cursor:"pointer",
            ...
            {[["tc","Total Cholesterol"],["hdl","HDL"],["ldl","LDL (Device Reported)"],["tg","Triglycerides"]].map(([k,lbl])=>(
              <div key={k}>
                ...
                <div onClick={()=>setPadField(k)} style={{padding:"11px 13px",borderRadius:9,
                  border:`1px solid ${padField===k?t.accent:t.border}`,background:t.bg,fontSize:17.5,
          ...
          {f.source==="Lab" && (
            <div>
              ...
              <div onClick={()=>setPadField("apobMeasured")} style={{padding:"11px 13px",borderRadius:9,
                ...
                fontSize:17.5,fontFamily:FONT,cursor:"pointer",minHeight:43,display:"flex",
          ...
          {(()=>{
            ...
            return (
              <div style={{background:t.bg,borderRadius:10,border:`1px solid ${t.border}`,padding:"0 14px 7px"}}>
                <div style={{fontSize:15,fontWeight:600,color:t.sec,letterSpacing:".5px",
                ...
                {rows.map((x,i)=>(
                  <div key={i} style={{display:"flex",alignItems:"baseline",justifyContent:"space-between",
                    ...
                    <div style={{fontSize:16.5,minWidth:0}}>
                    ...
                    <div style={{flexShrink:0,display:"flex",alignItems:"baseline",gap:4}}>
                      ...
                      {x.unit && <span style={{fontSize:15,fontWeight:600,color:t.muted}}>{x.unit}</span>}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
              ...
              <span style={{fontSize:15.5,color:t.muted}}>{f.notes.length}/300</span>
            </div>
...
function StatCard({ label, value, unit, provenance, color, trend, lowerBetter, onPress }) {
  ...
  if (trend && trend.delta !== 0) {
    ...
    trendEl = (
      <span style={{fontSize:15,fontWeight:600,fontFamily:MONO,color:col,
    ...
    <button onClick={onPress} aria-label={`${label}: choose which metric to show here`}
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",
        ...
        <span style={{display:"flex",alignItems:"center",gap:1,minWidth:0,flex:"1 1 auto"}}>
          <span style={{fontSize:15,color:t.sec,fontWeight:700,letterSpacing:"0.1px",
...
function DashboardView({ enriched, settings, onUpdateSettings, onSelect, onAdd, onViewAll }) {
  ...
  const MetricButton = ({ label, color, size, onClick }) => (
    <button onClick={onClick} style={{display:"inline-flex",alignItems:"center",gap:3,
      background:"none",border:"none",padding:0,cursor:"pointer",fontFamily:FONT,
      minWidth:0,maxWidth:"100%"}}>
      <span style={{fontSize:size,fontWeight:700,color,overflow:"hidden",
        textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{label}</span>
      <ChevronDown size={Math.round(size*0.8)} color={t.sec} style={{flexShrink:0}} />
    </button>
  );
    ...
    <div style={{paddingBottom:16}}>
      ...
      <div style={{background:t.card,borderRadius:14,padding:"18px 15px",marginTop:14,border:`1px solid ${t.border}`}}>
        <div style={{display:"flex",gap:8,alignItems:"baseline",flexWrap:"wrap",
          ...
          <MetricButton label={a?.label} color={a?.color} size={20}
            onClick={()=>setMetricPicker("primary")} />
          {bmB ? (
            <span style={{display:"inline-flex",alignItems:"center",gap:4,minWidth:0}}>
              <span style={{fontSize:15,color:t.sec}}>vs</span>
              <MetricButton label={b?.label} color={b?.color} size={15}
                onClick={()=>setMetricPicker("compare")} />
            </span>
          ) : (
            <button onClick={()=>setMetricPicker("compare")}
        ...
        {!enoughForChart ? (
          <div style={{height:216,display:"flex",flexDirection:"column",alignItems:"center",
            ...
            <div style={{fontSize:17,color:t.sec,textAlign:"center",padding:"0 20px"}}>
          ...
          <>
            <ResponsiveContainer width="100%" height={216}>
              <LineChart data={chartData} margin={{top:14,right:24,bottom:6,left:4}}>
                ...
                <YAxis yAxisId="left" tick={{fontSize:15,fill:bmB?(a?.color):t.sec,fontFamily:FONT}}
                ...
                {bmB && (
                  <YAxis yAxisId="right" orientation="right" domain={["auto","auto"]} width={36}
                    tick={{fontSize:15,fill:b?.color,fontFamily:FONT}} tickLine={false}
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",margin:"28px 4px 10px"}}>
        ...
        {enriched.length>0 && (
          <button onClick={onViewAll} style={{background:"none",border:"none",cursor:"pointer",
            color:t.text,fontSize:16,fontWeight:600,fontFamily:FONT,display:"flex",alignItems:"center",gap:1}}>
      ...
      {recent.length===0 ? (
        <div style={{background:t.card,borderRadius:14,padding:"36px 24px",textAlign:"center",
          ...
          <div style={{fontSize:17,color:t.sec,marginBottom:20}}>
          ...
          <button onClick={onAdd} style={{padding:"11px 22px",borderRadius:9,border:"none",
            background:t.accent,color:t.accentText,fontWeight:700,fontSize:17,cursor:"pointer",fontFamily:FONT}}>
    ...
    <div style={{paddingBottom:16}}>
      ...
      {groups.length===0 ? (
        <div style={{background:t.card,borderRadius:13,padding:"30px 20px",textAlign:"center",
          color:t.sec,fontSize:17,border:`1px solid ${t.border}`}}>
        ...
        return (
          <div key={month} style={{marginBottom:8}}>
            <div onClick={()=>toggle(month)} style={{display:"flex",alignItems:"center",gap:8,
              ...
              <span style={{fontSize:17,fontWeight:600,color:t.text,flex:1}}>{month}</span>
              <span style={{fontSize:15.5,color:t.sec,fontFamily:MONO}}>{rows.length}</span>
            </div>
            {!collapsed && (
...
function ReportView({ enriched, settings, onClose }) {
  ...
  const tableCell = { padding:"6px 8px",fontSize:15,borderBottom:`1px solid ${L.border}`,
    fontFamily:MONO,color:L.text,textAlign:"right" };
  const tableHd = { padding:"6px 8px",fontSize:15,fontWeight:700,textTransform:"uppercase",
  ...
  const ctrlBtn = (on)=>({ padding:"5px 11px",border:`1px solid ${on?"#18181B":"rgba(255,255,255,0.25)"}`,
    borderRadius:7,cursor:"pointer",fontFamily:FONT,fontSize:16,fontWeight:on?700:500,
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:260,background:"#26262A",
      ...
      <div className="ll-noprint" style={{position:"sticky",top:0,zIndex:2,background:"#1A1A1D",
        ...
        <span style={{fontSize:17,fontWeight:700,color:"#FAFAFA",marginRight:4}}>Report</span>
        ...
        <button onClick={()=>window.print()} style={{marginLeft:"auto",padding:"7px 16px",
          ...
          fontSize:16.5,cursor:"pointer",fontFamily:FONT,display:"flex",alignItems:"center",gap:6}}>
          <Download size={14} /> Save as PDF</button>
      </div>
      <div style={{textAlign:"center",fontSize:15,color:"#8A8A93",padding:"8px 16px 0"}}
        ...
        <div className="ll-report" style={{background:"#FFFFFF",width:720,maxWidth:"100%",
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",
            ...
            <div>
              ...
              <div style={{fontSize:16,color:L.sec,marginTop:3}}>{rangeLabel} · {srcLabel}</div>
            </div>
            <div style={{textAlign:"right",fontSize:15,color:L.sec}}>
              <div>Generated</div>
          ...
          {scoped.length===0 ? (
            <div style={{padding:"40px 0",textAlign:"center",color:L.sec,fontSize:16.5}}>
              No readings fall within this range and filter.
            </div>
          ) : (
            <>
              <div style={{fontSize:15,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.5px",
                ...
                <>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                    ...
                    <select value={chartBm} onChange={e=>setChartBm(e.target.value)}
                      className="ll-noprint" style={{fontSize:15,padding:"3px 6px",borderRadius:6,
                  ...
                  <div style={{marginBottom:24}}>
                    <LineChart width={632} height={220} data={chartData}
                      ...
                      <XAxis dataKey="date" tick={{fontSize:15,fill:L.sec,fontFamily:FONT}}
                        tickLine={false} axisLine={{stroke:L.border}} interval="preserveStartEnd"
                        minTickGap={40} tickMargin={8} />
                      <YAxis tick={{fontSize:15,fill:L.sec,fontFamily:FONT}} tickLine={false}
          ...
          <div style={{borderTop:`1px solid ${L.border}`,paddingTop:12,fontSize:15,
...
function SettingsView({ settings, onUpdate, enriched, onClearAll, onOpenReport,
  ...
  const Row = ({ label, value, sub, onPress, danger }) => (
    <div onClick={onPress}
      ...
      <div>
        ...
        {sub && <div style={{fontSize:15.5,color:t.sec,marginTop:2}}>{sub}</div>}
      </div>
      <div style={{display:"flex",alignItems:"center",gap:5}}>
        {value && <span style={{fontSize:17,color:t.sec,fontWeight:600}}>{value}</span>}
  ...
  const iS = { padding:"10px 12px",borderRadius:8,border:`1px solid ${t.border}`,
    background:t.bg,fontSize:17.5,outline:"none",fontFamily:FONT,color:t.text };
    ...
    <>
      <div style={{paddingBottom:16}}>
        ...
        {[
          ...
          return (
            <div key={pool}>
              ...
              <div style={{fontSize:15.5,color:t.sec,margin:"0 4px 8px",lineHeight:1.5}}>{blurb}</div>
              <div style={card}>
                {list.map((dn,i)=>{
                  ...
                  return (
                    <div key={dn}>
                      <div onClick={()=>onUpdate({...settings,[poolDefKey(pool)]:dn})}
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{fontSize:17,color:t.text}}>{dn}</div>
                          {isDefault && <div style={{fontSize:15,color:t.sec,marginTop:1}}>Default</div>}
                        </div>
                        <div style={{display:"flex",gap:4}}>
        ...
        <div style={{...card,padding:"14px 15px"}}>
          <div style={{fontSize:15.5,color:t.sec,marginBottom:10,lineHeight:1.55}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
              ...
              {["mg/dL","nmol/L"].map(u=>(
                <button key={u} onClick={()=>onUpdate({...settings,lpaUnit:u})}
                  style={{padding:"9px 10px",border:"none",cursor:"pointer",fontFamily:FONT,
                    fontSize:16,fontWeight:settings.lpaUnit===u?700:500,whiteSpace:"nowrap",
            ...
            <button onClick={()=>{onUpdate({...settings,lpa});setLpaSaved(true);}}
              style={{padding:"10px 14px",borderRadius:8,border:"none",cursor:"pointer",
                flexShrink:0,fontFamily:FONT,fontWeight:700,fontSize:17,
                background:t.accent,color:t.accentText}}>
              Save</button>
          </div>
          {settings.lpa && (
            <div style={{fontSize:16.5,color:lpaSaved?t.success:t.sec,marginTop:8}}>
        ...
        <div style={card}>
          <div onClick={onOpenReport}
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <span style={{fontSize:17,color:t.text}}>PDF Report</span>
            </div>
          ...
          <div onClick={()=>enriched.length && exportCSV(enriched,settings)}
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <span style={{fontSize:17,color:t.text}}>Export CSV</span>
            </div>
            <span style={{fontSize:16.5,color:t.sec}}>{enriched.length} readings</span>
          </div>
        </div>
...
export default function App() {
  ...
  if (!loaded) return (
    <div style={{display:"flex",height:"100vh",alignItems:"center",justifyContent:"center",
      background:t.bg,fontFamily:FONT,color:t.sec,fontSize:17.5}}>Loading…</div>
  );
    ...
    <ThemeCtx.Provider value={t}>
      ...
      <div style={{minHeight:"100vh",background:t.bg,fontFamily:FONT,color:t.text,
        ...
        <header style={{background:`${t.bg}E8`,backdropFilter:"blur(20px)",
          ...
          <div style={{fontSize:18,fontWeight:800,letterSpacing:"-0.4px"}}>LipidLog</div>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            {view!=="settings" && (
              <button onClick={()=>setModal("add")} style={{background:t.accent,color:t.accentText,
                border:"none",borderRadius:18,padding:"6px 13px 6px 9px",fontSize:17,fontWeight:700,
        ...
        <main style={{maxWidth:600,margin:"0 auto",padding:"0 12px"}}>
        ...
        <nav style={{position:"fixed",bottom:0,left:0,right:0,background:`${t.card}F2`,
          ...
          {TABS.map(({key,label,Icon})=>(
            <button key={key} onClick={()=>setView(key)} style={{background:"none",border:"none",
              ...
              <span style={{fontSize:15,fontWeight:view===key?700:500}}>{label}</span>
            </button>
          ))}
        </nav>
